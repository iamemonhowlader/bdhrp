        ...
        <div class="container-fluid px-4 pt-4 pb-5">
            ...
            <div class="premium-page-header">
                <div class="row align-items-center">
                    <div class="col-lg-8">
                        <span class="badge bg-white text-success mb-3 p-2 rounded-pill fw-bold shadow-sm">EDIT TOPIC</span>
                        <h1 class="display-5 fw-800 mb-2 mt-2">{{ $topic->title }}</h1>
                        <p class="fs-5 opacity-90 mb-0">Update the information profile for this human rights topic.</p>
                    </div>
                </div>
            </div>
                    ...
                    <div class="col-lg-8">
                        <div class="glass-editor-card">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="title" class="form-control premium-input shadow-sm @error('title') is-invalid @enderror" value="{{ old('title', $topic->title) }}" placeholder="e.g. Children's Rights">
                                @error('title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>

                            <div class="mb-0">
                                <label class="form-label-premium">DESCRIPTION</label>
                                <textarea name="description" id="editor" class="form-control premium-input shadow-sm @error('description') is-invalid @enderror" rows="10" placeholder="Detailed description of the topic...">{{ old('description', $topic->description) }}</textarea>
                    ...
                    <div class="col-lg-4">
                        <div class="glass-editor-card">
                            ...
                            @if($topic->image)
                                <div class="current-image-preview">
                                    <img src="{{ asset('storage/' . $topic->image) }}" class="w-100 h-auto">
                                </div>
                            @endif
                            <div class="mb-0">
                                <label class="form-label-premium">UPDATE IMAGE</label>
                                <input type="file" name="image_file" class="form-control premium-input shadow-sm @error('image_file') is-invalid @enderror">
                                <small class="text-muted mt-2 d-block">Leave blank to keep current image. Max 2MB.</small>
                        ...
                        <div class="glass-editor-card">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="sort_order" class="form-control premium-input shadow-sm @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $topic->sort_order) }}">
                                @error('sort_order') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            </div>
                            <div class="form-check form-switch mb-0">
                                <input class="form-check-input" type="checkbox" name="is_active" id="activeSwitch" value="1" {{ $topic->is_active ? 'checked' : '' }}>
                        ...
                        <div class="d-grid shadow-lg rounded-pill overflow-hidden">
                            <button type="submit" class="btn btn-save py-3">
                                <i class="bi bi-cloud-check-fill me-2"></i> UPDATE TOPIC
                            </button>
                        </div>
                    </div>
                </div>
